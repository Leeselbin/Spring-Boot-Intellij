package com.example.projtest.aop;
...
public class PerformanceAspect {
    ...
    // 특정 어노테이션을 대상 지정
    @Pointcut("@annotation(com.example.projtest.annotation.RunningTime)")
    private void enableRunningTime() {}

    // 기본 패키지의 모든 메소드
    @Pointcut("execution(* com.example.projtest..*.*(..))")
    private void cut(){}

    // 실행 시점 설정: 두 조건을 모두 만족하는 대상을 전후로 부가 기능을 삽입입
